    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center w-full h-full overflow-hidden backdrop-blur-sm">
      ...
      >
        ...
        <div className="p-2">
          ...
          <p style={{ fontSize: "15px" }} className="subtext mb-3">
            {description}
          </p>
          {subDescription.map((s) => (
            <p style={{ fontSize: "13px" }} className="subtext ">
              {s}
            </p>
          ))}
            ...
            <div className="flex gap-3">
              {tags.map((t) => (
                <img
                  key={t.id}
                  src={t.path}
                  className="rounded-3xl size-10 hover:animate-bounce cursor-pointer"
                />
              ))}
            </div>
            <a
              className="inline-flex gap-2 text-black cursor-pointer items-center"
              href={href}
            >
              View Code
              <img
